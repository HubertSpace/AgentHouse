...
use ah_control::{
    ...
    TerminalKeyInputModifiers, UiLanguagePreference, UiThemeSchemePreference, WindowSplitDirection,
    WindowSummary, WindowTabSummary, WorkspaceSummary,
};
...
fn wait_for_web_preview(
    ...
    loop {
        ...
        let has_frame = snapshot
            .get("browser_surface")
            .and_then(|surface| surface.get("frame"))
            .is_some_and(|frame| !frame.is_null());
        let is_ready = has_page && (browser_status == "Loaded" || has_frame);
        if is_ready {
            return Ok(surface);
        }
        if started.elapsed() > timeout {
            bail!(
                "timed out waiting for web preview {url}; browser={browser_status} preview={preview_status} page_url={page_url:?} frame={has_frame}"
            );
        }
        thread::sleep(Duration::from_millis(250));
    }
...
fn parse_ui_theme_scheme_preference(arg: Option<&String>) -> Result<UiThemeSchemePreference> {
    ...
    UiThemeSchemePreference::from_control_code(value).with_context(|| {
        format!(
            "unsupported UI color scheme: {value}; expected one of cream, warm, blue, green, red, purple, glass, luxury, soft"
        )
    })
}
...
mod tests {
    ...
    fn parses_ui_theme_scheme_cli_args() {
        ...
        assert_eq!(
            parse_ui_theme_scheme_preference(Some(&"blue".to_string())).unwrap(),
            UiThemeSchemePreference::Blue
        );
