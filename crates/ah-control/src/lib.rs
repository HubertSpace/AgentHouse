...
pub struct PersistedControlState {
    ...
    #[serde(default)]
    pub closed_workspace_ids: Vec<WorkspaceId>,
    #[serde(default)]
    pub pane_layouts: Vec<PersistedPaneLayout>,
    #[serde(default)]
...
pub struct PersistedPaneLayout {
    ...
    pub root: PersistedPaneLayoutNode,
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(tag = "type", rename_all = "snake_case")]
pub enum PersistedPaneLayoutNode {
    Leaf {
        window_id: WindowId,
    },
    Split {
        ...
        first: Box<PersistedPaneLayoutNode>,
        second: Box<PersistedPaneLayoutNode>,
    },
}
...
impl UiThemeSchemePreference {
    pub const ALL: [Self; 1] = [Self::Glass];
...
#[derive(Clone, Copy, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum UiThemeModePreference {
    System,
    Light,
    Dark,
}

impl UiThemeModePreference {
    ...
    pub fn from_control_code(value: &str) -> Option<Self> {
        match value.trim().to_ascii_lowercase().as_str() {
            "system" | "auto" | "跟随系统" | "系统" => Some(Self::System),
            "light" | "magazine" | "杂志" | "浅色" | "亮色" => Some(Self::Light),
            "dark" | "glass" | "深色" | "暗色" => Some(Self::Dark),
            _ => None,
        }
    }
}
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct UiThemeModeOptionSummary {
    pub preference: UiThemeModePreference,
    pub code: String,
    pub label: String,
    pub native_label: String,
}

impl UiThemeModeOptionSummary {
    #[must_use]
    pub fn from_preference(preference: UiThemeModePreference) -> Self {
        Self {
            preference,
            code: preference.control_code().to_string(),
            label: preference.label().to_string(),
...
pub struct AppSettingsSummary {
    ...
    pub ui_theme_mode_options: Vec<UiThemeModeOptionSummary>,
}
    ...
    pub fn new(
        ...
        Self {
            ...
            ui_theme_scheme_options: UiThemeSchemePreference::all()
                ...
                .collect(),
            ui_theme_mode: ui_theme_mode.control_code().to_string(),
            ui_theme_mode_options: UiThemeModePreference::all()
                .iter()
                .copied()
                .map(UiThemeModeOptionSummary::from_preference)
                .collect(),
        }
    }
}
...
mod tests {
    use super::{
        ...
        UiThemeModePreference, UiThemeSchemePreference, WindowSplitDirection, WindowTabSummary,
    };
    ...
    fn app_settings_summary_exposes_language_control_options() {
        ...
        assert_eq!(summary.ui_theme_scheme_options.len(), 1);
        assert_eq!(summary.ui_theme_scheme_options[0].code, "glass");
        assert_eq!(
            summary.ui_theme_scheme_options[0].native_label,
            "杂志 / Glass"
        );
        assert_eq!(summary.ui_theme_mode, "light");
        assert_eq!(summary.ui_theme_mode_options.len(), 3);
        assert_eq!(summary.ui_theme_mode_options[0].code, "system");
        assert_eq!(summary.ui_theme_mode_options[1].code, "light");
        assert_eq!(summary.ui_theme_mode_options[2].code, "dark");
        ...
        assert_eq!(json["ui_theme_scheme_options"][0]["code"], "glass");
        assert_eq!(
            json["ui_theme_scheme_options"][0]["native_label"],
            "杂志 / Glass"
        );
        assert_eq!(json["ui_theme_mode"], "light");
        assert_eq!(json["ui_theme_mode_options"][2]["code"], "dark");
    }
    ...
    fn decodes_persisted_control_state_without_ui_settings() {
        ...
        assert!(state.closed_workspace_ids.is_empty());
        assert!(state.pane_layouts.is_empty());
    }

    #[test]
    fn encodes_persisted_pane_layouts() {
        let workspace_id = WorkspaceId::new();
        ...
        let state = super::PersistedControlState {
            ...
            pane_layouts: vec![super::PersistedPaneLayout {
                workspace_id,
                root: super::PersistedPaneLayoutNode::Split {
                    id: 7,
                    direction: WindowSplitDirection::Down,
                    ratio_basis_points: 4250,
                    first: Box::new(super::PersistedPaneLayoutNode::Leaf { window_id: first }),
                    second: Box::new(super::PersistedPaneLayoutNode::Leaf { window_id: second }),
                },
            }],
            ui_language: None,
            ui_theme_scheme: None,
            ui_theme_mode: None,
            next_event_sequence: 1,
            events: Vec::new(),
            session_rings: Vec::new(),
        };

        let json = serde_json::to_value(&state).expect("state should encode");
        assert_eq!(json["pane_layouts"][0]["root"]["type"], "split");
        assert_eq!(json["pane_layouts"][0]["root"]["direction"], "down");
        assert_eq!(json["pane_layouts"][0]["root"]["ratio_basis_points"], 4250);

        let restored: super::PersistedControlState =
            serde_json::from_value(json).expect("state should decode");
