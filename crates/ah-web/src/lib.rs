...
fn fetch_http_text_snapshot(
    ...
    let mut response = agent
        .get(url)
        .header("User-Agent", "AgentHouse/0.1")
        .call()
...
mod tests {
    use super::{
        BrowserAction, BrowserEngine, BrowserLoadStatus, BrowserSessionState, PageSnapshot,
        Timestamp, extract_html_text, validate_http_url,
    };
